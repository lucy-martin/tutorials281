...
A **martix** is an object in R in which 1) every element is the same type (e.g., all numbers or all characters), and 2) is arranged in a two-dimensional format.


Because matrices only handle data of the same type, we will not be using them very much. But, it is important to understand what they are and become familiar with a few commands.
...
It is not so important for us to dive into this more right now, as we will not be relying on matrices too much in this class. So, let's move onto data frames!


## Topic 2: Data Frames

Data frames are also 2D types of data, however they are more flexible than matrices. Instead of only being able to handle *one* type of data, they can handle multiple types, including numbers, text, and Boolean data types.
...
After you create this data frame, use `dim()` to check that your data frame has the correct number of rows and columns: 4 and 4!
