...
Factors are a data type specifically designed for handling categorical data in R. Categorical variables are variables that represent data in different categories or levels. There are two main types of categorical variables: nominal and ordinal.
...
In R, we use the `factor()` function to create factors. Let's see how it works using party affiliation as an example:
...
As you can see, `party.factor` has two levels: Democrat and Republican. We can also inspect the structure of R objects, including factors, using the `str()` function:

```{r eval = T}
str(party.factor) # inspect the structure of the factor
```

The output shows that R recognizes party.factor as a factor with two levels: Democrat (assigned a 2) and Republican (assigned a 1). Sometimes we may want to change the order of the levels in a factor. For example, let's make Republican the first level and Democrat the second:
...
As noted above, party affiliation is a nominal variable. When dealing with ordinal variables, we need to create ordered factors that reflect the natural order of the categories. Let's take the example of language proficiency levels:

```{r eval = T}
proficiency <- c("Advanced", "Beginner", "Intermediate", "Intermediate", "Advanced") #create a vector
proficiency.factor <- factor(proficiency,ordered = TRUE, levels = c("Beginner", "Intermediate", "Advanced")) # create factor
